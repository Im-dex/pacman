...
Value::Value(const std::string& data)
{
    ...
    PACMAN_CHECK_ERROR(parseResult && mRoot.isObject(), ErrorCode::BadFormat);
}
...
{
    PACMAN_CHECK_ERROR(mRoot.isString(), ErrorCode::BadFormat);
...
{
    PACMAN_CHECK_ERROR(mRoot.isNumeric(), ErrorCode::BadFormat);
...
{
    PACMAN_CHECK_ERROR(mRoot.isBool(), ErrorCode::BadFormat);
    return mRoot.asBool();
}
...
{
    PACMAN_CHECK_ERROR(mRoot.isArray(), ErrorCode::BadFormat);
    return Array(mRoot);
}
...
Value Value::GetValue<Value>(const std::string& name) const
{
    ...
    PACMAN_CHECK_ERROR(val.isObject(), ErrorCode::BadFormat);
    return Value(val);
}
...
{
    PACMAN_CHECK_ERROR(index < GetSize(), ErrorCode::BadArgument);
